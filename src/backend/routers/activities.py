...
def get_activities(
    ...
    if difficulty:
        # Include activities matching the chosen level AND those with no difficulty set
        difficulty_condition = {"$or": [
            {"difficulty": difficulty},
            {"difficulty": {"$exists": False}}
        ]}
        # Wrap with $and so the difficulty condition is combined with any other filters
        if query:
